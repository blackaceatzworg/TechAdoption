...
/**
 * Manages the collection of ReinforcementLearners in a simulation. Responsible for 
 * communicating parameter settings from a settings editor to individual leaners. 
 *  
 * 
...
public class BasicLearnerManager {
	...
	/*
	 * Agents are grouped according to their learning algorithm, as indicated 
	 * by the Class of ReinforcementLearner they use. This Hashtable maintains
	 * the grouping.
	 */
	...
	public enum RegistrationEvent {REGISTERATION, UNREGISTERATION, BATCH_REGISTRATION}
	...
	public BasicLearnerManager(ReinforcementLearner[] learnerList){
		...
		if((learnerList != null) && (learnerList.length != 0)){
			for(ReinforcementLearner learner: learnerList){
				if(learner == null){
					System.err.println("Warning: BasicLearnerManager recieved a null " +
	 ...
	 * If it is, then this will return the String id that it is currently regisitered under.
	...
	/**
	 * Register a bare ReinforcementLearner without a given ID. This learner to be 
	 * registered under a generated generic ID. The ID will be returned upon successul
	 * registration.
	 * 
	 * @param learner - the bare learner to register
	 * @return the auto-generated ID for this leaner. If the learner is already registered,
	 * returns the existing ID assigined to this anonymous learner. Returns null if the 
	 * learner is not registered successfully. 
	 */
	 ...
	 * 
	 * @param id - the String indentifier that the learner will be registered under.
	 
	 * @param learner
	 * @return true if the learner was successfully registered, false if a the learner
	 * has already been registered or another learner is registered under the given id. 
	 */
	public boolean register(String id, ReinforcementLearner learner){
		...
		//Wrap the learner in a dummy agent so that it may be added to the registery
		edu.iastate.jrelm.util.WrapperAgent wrapAgent = 
			new edu.iastate.jrelm.util.WrapperAgent(id, learner);
	 ...
	 * @return true if the agent was successfully registered, false if a the agent
	 * has already been registered or another agent is already registered under the 
	 * same ID.
	 */
	 ...
	 * 
	 * @param agents
	 * @return true if all agents were successfully registered, false if one or more 
	 * agents were already registered or another agent is already registered under the 
	 * same ID. 
	 */
	public boolean register(Collection<JReLMAgent> agents){
		...
		for(JReLMAgent<ReinforcementLearner> agent: agents){
			//if an agent is registered successfully then add it's id to the list of 
			//newly registered agents. Otherewise, mark failure and move on to the 
			//next agent
			if(register(agent)){
				ids.add(agent.getID());
			}else{
		...
		notifyListeners(ids.toArray(new String[0]), RegistrationEvent.BATCH_REGISTRATION);
	 ...
	 * 
	 * @param learner - remove this ReinforcementLearner from the
	 * 					registry
	 * @return true if the learner was successfully unregistered. False if the learner
	 * 			was not found in the registery.
	 */
	...
	/**
	 * Agents are grouped according to their learning algorithm as indicated 
	 * by the type of RLParameters their learner uses. This method places the given 
	 * agent in the appropriate algorithm group.   
	 */
	protected void groupAgent(JReLMAgent agent){
		Class learnerClass;
		
		//If the agent has a SimpleStatelessLearner, dig one more level to get the
		//true learner claas
		if(SimpleStatelessLearner.class.isInstance(agent.getLearner())){
			learnerClass = ((SimpleStatelessLearner)agent.getLearner()).getEngine().getClass();
		}else
			learnerClass = agent.getLearner().getClass();

		Vector<JReLMAgent> group;
		
		//If a group for this algorithm exists, then add the agent to the group,
	...
	/**
	 * Agents are grouped according to their learning algorithm as indicated 
	 * by the type of RLParameters their learner uses. This method removes the 
	 ...
	 * 
	 * @param id - the String identifier for the desired agent
	 * @return true if the settings were accepted by the agent's learner. False if the 
	 * 	RLParameters given were incompatible with the agent's learner.
	 */
	public boolean updateSettings(String id, RLParameters params){
		...
		if(agent ==  null){
			System.err.println("Tried to set parameters for agent '"+ id +
		...
		try{	
			...
			System.err.println("JReLM BasicLearnerManager update error: Attempted to set " +
					"incompatible parameters for ID '" +id+"'.");
			System.err.println(params.getClass().getName() +" not compatible with " + learnerClass);
	...
	/**
	 * Update the settings for all agents in the algortithm group indicated by the 
	 * given Class of ReinforcementLearner. The given RLParameters must be compatible 
	 * the indicated ReinforcementLearner type.
	 * 
	 * @param learnerType - Class of ReinforcementLearner indicating which group of agents should 
	 * 			receive these settings 
	 * @param params - the new learning parameter settings to use for all agents in the
	 * 			group
	 * @return true if the given parameters have been accepted by all agents in the group.
	 * 			False if the parameter settings were rejected by any agent in the group.
	 */
		...
		if(group != null){
			for(JReLMAgent agent: group){
				try{	
					...
					Class learnerClass = agent.getLearner().getClass();
					System.err.println("JReLM BasicLearnerManager update error: Attempted to set " +
							"incompatible parameters for ID '" +agent.getID()+"'.");
					System.err.println(params.getClass().getName() +" not compatible with " + learnerClass);
					System.err.println("Using previous settings.");
	...
	/**
	 * Retrieve an agent with the given ID. If no such agent is registered, then null 
	 * is returned.
	 * 
	 * @param agentID - the ID of the desired agent
	 * @return an agent with the given ID or null if no such agent has been registered.
	 */
	...
	/**
	 * Retrieves a group of agents that all use the same reinforcement learning algorithm
	 * as indicated by the given class of ReinforcementLearner.
	 * 
	 * @param learnerClass
	 * @return
	 */

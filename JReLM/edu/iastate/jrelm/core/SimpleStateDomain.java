...
import edu.iastate.jrelm.core.SimpleState;
	 ...
	 * SimpleStates that wrap a given collection of Objects that represent
	 * state choices. 
	 * <P>
	 * This domain builds itself from a given collection of Objects. Each Object 
	 * is considered to represent a state of the environment external to the 
	 * ReinforcementLearning (or agent). The Collection is the list
	 * of all possible states an ReinforcementLearner may encounter. 
	 * Internally, each Object is wrapped in SimpleState and the space of possible
	 * states is organized by Integer index. The use of SimpleState wrappers allow 
	 * the domain to be used with other learning components (i.e. 
	 * ReinforcementLearners that implement stateful learning algorithms). 
	 *<P>
	 * Note, this class is only appropriate for contexts where the set of all 
	 * possible states is discrete and finite.
	 * 
	 * @param <O> - This is the original type of state Objects in
	 * 				the given collection.
	 */
	public class SimpleStateDomain<O> 
			implements StateDomain<Integer, SimpleState<O>>{
		 ...
		 * @param states - list of objects specifying states. 
		 */
		 ...
		 * 
		 * @param stateIndex - int index of the desired state in the domain
		 * @return - the desired state (note: the original object is wrapped in 
		 * 			 a SimpleState)
		 */

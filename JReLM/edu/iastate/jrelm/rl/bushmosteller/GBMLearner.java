...
public class GBMLearner<I, A extends Action<I>> 
		extends AbstractStatlessLearner<GBMParameters, 
										I,
										A,
										Feedback<Double>,
	...
	public void update(Feedback<Double> reward) {
		...
		if(reward.getValue() >= 0.0) {
			...
			modifiedReward = 
				parameters.feedbackModifier(reward.getValue());
			...
			modifiedReward = 
				parameters.feedbackModifier(-1 * reward.getValue());
			...
			for(I actionID : idList) {
				...
				if(actionID == lastChosen) {
					...
					adjustmentValue = modifiedReward * 
						( (currentProbability * getPolicy().getProbability(lastChosen)) / 
								(1 - getPolicy().getProbability(lastChosen)) );
				}
	...
	/**
	 * The implementation of Generalised Bush-Mosteller learning is 
	 * partially distributed between the GBMLearnerTest and a specific 
	 * child of GBMParameters. As such, the parameters returned here 
	 * will determine a specific variation of Generalised Bush-Mosteller 
	 * learning.
	 * 
	 * In particular a new LinearGBMParameters is returned. Using these 
	 * parameters will result in Generalised Bush-Mosteller learning based on
	 * a linear feedback modificatier function.
	 * 

 ...
 * child classes should specifiy a reinforcement strength modifier function 
 * that will be used in updating Policies. This is done by implementing the 
 * abstract feedbackModifier(double) function. In effect, we are distributing 
 * the implementation of Generalized Bush-Mosteller learning algorithms accross
 * the parameters and learner objects.
 * 
...
public abstract class GBMParameters implements RLParameters {
	 ...
	 * resulting from the last action chosen. Essentially v(r) yields a 
	 * modified feedback value, associated with the last selected action 
	 * choice, that is then used in updating a Policy according to generalize
	 * Bush-Mosteller learning.
	 * 
	 * Note, v(r) must be monotonically increasing in r, for r > 0, where v(0) = 0
	 * and 0 <= v(r) <= 1.
	 * 

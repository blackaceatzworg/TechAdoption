 ...
 * is constrained so that v(0) = 0 and 0 <= v(r) <= 1. This means the 
 * multiplier value v in v(r) = v * r must be chosen such that (v * r_min) >= 0 
 * and (v * r_max) <= 1, where r_min and r_max are the minimum and maximum 
 * expected values for reinforcement strengths. 
 * <p>
 * If you wish to use LinearGMBParameters to check that the current setting 
 * for the multiplier value v is valid, you must first set the min and max 
 * expected reinforcement strengths through setFeedbackRange(min, max).
 * 
...
public class LinearGBMParameters extends GBMParameters {
	 ...
	 * multiplier value v is v = 1.0. The default settings for the minimum
	 * and maximum expected reinforcement strength are 0.0 and 1.0 
	 * respectively.
	 * 
	 ...
	 * construction if you wish to confirm that the settings are appropriate
	 * for Linear Generalized Bush-Mosteller learning.
	 * 
	...
	public boolean validateParameters() {
		...
		if( (scaledMin >= 0.0) && (scaledMin <= 1.0) && 
		    (scaledMax >= 0.0) && (scaledMax <= 1.0)) {
			valid = true;
		}
	...
	/**
	 * Retrieve the current setting for the multiplier value v used in the feedback
	 * modifier function v(r) = v * r.
	 */
	...
	/**
	 * Set the current multiplier value v used in the feedback
	 * modifier function v(r) = v * r.
	 * 
	 ...
	 * the multiplier value v in v(r) = v * r must be chosen such that 
	 * (v * r_min) >= 0 and (v * r_max) <= 1, where r_min and r_max are the 
	 * minimum and maximum expected values for reinforcement strengths. 
	 */

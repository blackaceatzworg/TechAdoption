...
public class RELearner<I, A extends Action<I>> 
		 extends AbstractStatlessLearner<REParameters, 
		 								 I, 
		 								 A, 
		 								 Feedback<Double>, 
		 								 REPolicy<I,A>> {

	//Number of possible Actions according to the ActionDomain of the LearingPolicy
	protected int domainSize = -1;
	
	/*
	 * List of action ID's in the domain. Allows us to map from propensities to Actions
	 * in the domain
	 */
	 ...
	 * 
	 * @param learningParams - the collection of parameter settings for the 
	 * 						RELearner
	 * @param aDomain - the ActionDomain to learn over
	 */
	 ...
	 * @param learningParams - the collection of parameter settings for the 
	 * 						RELearner
	 * @param aPolicy - a Roth-Erev specific policy type (REPolicy).
	 */
	 ...
	 ************************************************************************************/
	/*
	 * Update the propensities for all Actions. The propensity for last Action chosen
	 * will be updated using the feedback value that resulted from performing the Action.
	 *
	 * If j is the index of the last action chosen, r_j is the reward received for performing j, 
	 * i is the current action being updated, q_i is the propensity for i, and phi is the recency
	 * parameter, then this update function can be expressed as
	 * 	
...
//System.out.println("	"+i+ " previous: "+ policy.getPropensity(i));
...
//System.out.println(" new: "+policy.getPropensity(i));
	 ...
	 * come into play. That is, all action choices are assumed to be equally similar. 
	 * If the actionIndex points to the action the reward is associated with (usually 
	 * the last action taken) then simply adjust the weight by the experimentation. 
	 * Otherwise, adjust the weight by a smaller portion of the reward.
	 * 
	 *  If j is the index of the last action chosen, r_j is the reward received for performing j, 
	 *  i is the current action being updated, n is the size of the action 
	 *  domain and e is the experimentation parameter, then this experience 
...
//out.println("	actionIDList contains lastAction: "+ actionIDList.contains(lastSelectedAction.getID()));
		int rewardedIndex = actionIDList.indexOf(getLastSelectedAction().getID());
//out.println("	rewardedIndex: "+ rewardedIndex);
//out.println("	domainSize: " + domainSize);
//out.println("	reward * (1-e) == " + reward * (1-e));
//out.println("	reward * (e /(domainSize - 1)) == " + reward * (e /(domainSize - 1)));
		if(actionIndex == rewardedIndex){
//out.println("	this action was last selected");			
			experience = reward * (1-e);
		}else{
//out.println("	prop: "+policy.getPropensity(actionIndex)+ " experiment update: " + (e /(domainSize - 1)));			
...
//out.println("	experience: "+ experience);	
	 ...
	 ************************************************************************************/	
	/**
	 * Updates the probability for each action to be chosen in the policy. Uses a 
	 * proportional probability unless the given parameters say to use a 
	...
	/*
	 * Generate action probabilities using a Boltzmann distribution with a constant
	 * temperature
	 * 
	 */
...
//System.out.println("RELearner.generateBoltzmannProbs()");
		...
		//TODO: What was I thinking here? Perhaps when this was in VRELearner. See if this is still needed.
...
//System.out.println("	prop: " + prop + "  exp:" + Math.exp((prop/coolingParam)) + "  sum: " + summedExps);
		}
...
//System.out.println("	top: " + Math.exp(propensities[index]/coolingParam));			
...
//System.out.println("  new: " + policy.getProbability(actID));			
	...
	/**
	 * This activates the learning process according to the modified Roth-Erev learning 
	 * algorithm. Feedback is interpreted as reward for the last Action chosen by this 
	 * engine. Entries in the policy associated with this Action are updated accordingly.
	 * <P>
	 * This algorithm expects feedback to be given as a Double.
	 * 
	 * @param feedback - reward for the specified action
	 *
	 ...
	 * 
	 * @param feedback - reward for the specified action, given as a primitive
	 * 					 double.
	 *
	 * @see edu.iastate.jrelm.rl.ReinforcementLearner
	 */
	 ...
	 * @param params - learning parameters of type REParameters
	 * 

...
public class VRELearner<I, A extends Action<I>> 
	 ...
	 * ***********************************************************************/
	/**
	 * Construct a learner using Variant Roth-Erev with parameters specified in a
	 ...
	 * @param learningParams - the collection of parameter settings for the 
	 * 						   VRELearner
	 * @param aPolicy - a SimpleStatelessPolicy
	 */
	 ...
	 * associate the given feedback with the given Action. Learning will
	 * proceed in accordance with the Variant Roth-Erev algorithm, however, the
	 * given Action will be treated as if it were the last one selected by the 
	 * last call to chooseAction().
	 * <P>
	 ...
	 * about the given Action. That is, an Exception will be thrown if the 
	 * given Action is not in the ActionDomain that this learner is using.  
	 * 
	 * @param feedback - reward for the specified action, given as a primitive
	 * 					 double.
	 * 
	 * @param actionToReinforce - update will proceed associating the given
	 *                   		  feedback with this Action
	 */
	public void update(Feedback<Double> feedback, A actionToReinforce) throws Exception {
		
		// If we don't know about this action then complain
		if(!getPolicy().getActionDomain().containsAction(actionToReinforce)) {
			Exception e = new Exception("VRELearner.update(Feedback, Action) " +
	 ...
	 * 
	 * @see edu.iastate.jrelm.rl.rotherev.variant.VRELearner#update(Feedback, Action)
	 * 
	 * @param feedback - reward for the specified action, given as a primitive
	 * 					 double.
	 * 
	...
	public void update(double feedback, A actionToReinforce) throws Exception{
		...
		// If we don't know about this action then complain
		if(!getPolicy().getActionDomain().containsAction(actionToReinforce)) {
			Exception e = new Exception("VRELearner.update(double, Action) " +
					"received an Action that is not in the ActionDomain " +
					"associated with this learner's Policy. This learner does" +
					" not know about Action " + 
					actionToReinforce.getID().toString() +".");
	 ...
	 * i is the current action being updated, q_i is the propensity for i, n is the size 
	 * of the action domain and e is the experimentation parameter, then this experience function 
	 * can be expressed as
	 *   			 _
	 *  			|  r_j * (1-e) 		if i = j
	 *  E(i, r_j) = | 
...
//out.println("	actionIDList contains lastAction: "+ actionIDList.contains(actionIndex));
...
//out.println("	prop: "+policy.getPropensity(actionIndex)+ " experiment update: " + (e /(domainSize - 1)));			
			experience = getPolicy().getPropensity(actionIndex) * (e /(domainSize - 1));
		}

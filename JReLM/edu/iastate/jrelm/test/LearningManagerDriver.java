...
public class LearningManagerDriver {
	...
	/* public static void main(String[] args) {
		...
		VRELearner reLearner1 = 
			new VRELearner(new VREParameters(), domain);
		WrapperAgent reAgent1 = new WrapperAgent("RothErevAgent1", reLearner1);
		
		VRELearner reLearner2 = 
			new VRELearner(new VREParameters(), domain);
		WrapperAgent reAgent2 = new WrapperAgent("RothErevAgent2", reLearner2);
		
		ARELearner areLearner1 = 
			new ARELearner(new AREParameters(), domain);
		WrapperAgent areAgent1 = new WrapperAgent("AdvancedRothErevAgent1", areLearner1);
		
		FakeQLearner qLearner1 = new FakeQLearner(new FakeQLearningParameters(), domain);
		WrapperAgent qAgent1 = new WrapperAgent("Curly", qLearner1);

		FakeQLearner qLearner2 = new FakeQLearner(new FakeQLearningParameters(), domain);
		WrapperAgent qAgent2 = new WrapperAgent("Moe", qLearner2);
		
		FakeQLearner qLearner3 = new FakeQLearner(new FakeQLearningParameters(), domain);
		WrapperAgent qAgent3 = new WrapperAgent("Jack", qLearner3);
		
		SimpleStatelessLearner sLeaner1 = new SimpleStatelessLearner(new VREParameters(), temp);
		...
		//test registration 
		VRELearner rawLearner1 =
			new VRELearner(new VREParameters(), domain);
		ARELearner rawLearner2 =
			new ARELearner(new AREParameters(), domain);
		ARELearner rawLearner3 =
			new ARELearner(new AREParameters(), domain);
...
//		manager.updateSettings("RothErevAgent1", newQParams);
		...
		VREParameters params = (VREParameters) agent.getLearner().getParameters();

//System.out.println("*** TestManager ****");
//System.out.println("Params for RothErevAgent1");
//System.out.println("Experimentation: " + params.getExperimentation());
//System.out.println("Initiail Propensity: " + params.getInitialPropensity());
//System.out.println("Number Of Actions: " + params.getNumberOfActions());

...
public class TestModifiedEmpiricialWalker extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		eventGen = new ModifiedEmpiricalWalker(pdf, Empirical.NO_INTERPOLATION, randEngine);
	}
...
//			System.out.println("	event "+k+": " + (double)counts[k]/(double)numTrials);
	}
...
System.out.println("TestEmpiricalWalker.testNextDouble()");
		eventGen = new ModifiedEmpiricalWalker(pdf, Empirical.LINEAR_INTERPOLATION, this.randEngine);
		int numEvents = pdf.length;
		int nextEvent;
		for(int j=1; j <= numTrials; j++){
			nextEvent = eventGen.nextInt();

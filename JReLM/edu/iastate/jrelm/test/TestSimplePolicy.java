...
public class TestSimplePolicy extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		policy = new SimpleStatelessPolicy<Integer, SimpleAction<Integer>>(domain, distrib);
	}
	...
	public void testSimplePolicydomain() {	
		policy = null;
		policy = new SimpleStatelessPolicy<Integer, SimpleAction<Integer>>(domain);
		assertNotNull(policy);
	}
	
	public void testSimplePolicydomainRandomEngine(){
		policy = null;
		policy = new SimpleStatelessPolicy<Integer, SimpleAction<Integer>>(domain, randomGen);
		assertNotNull(policy);
	...
	public void testSimplePolicydomainDistrib(){
		...
		policy = null;
		try{
			policy = new SimpleStatelessPolicy<Integer, SimpleAction<Integer>>(domain, newDistrib);	
	...
	public void testSimplePolicydomainDistribRandomEngine(){
		policy = null;
		policy = new SimpleStatelessPolicy<Integer, SimpleAction<Integer>>(domain, distrib, randomGen);
		assertNotNull(policy);
	...
	public void testGenerateAction() {
		...
		for(int i=0; i < numCalls; i++){
			action = policy.generateAction();
System.out.println("	 "+action.getID().intValue()+": "+ policy.getProbability(action.getID()));			
		}
...
System.out.println("	 "+action.getID().intValue()+": "+ policy.getProbability(action.getID()));			
	...
	public void testSetDistribution(){
		...
		try{
			...
			//Check that both the distribution in use and getProbability have the same values
			//as the previously set distribution. That is the distribution has not changed since

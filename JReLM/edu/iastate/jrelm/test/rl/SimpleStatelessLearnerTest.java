...
public class SimpleStatelessLearnerTest extends TestCase {
	...
	ReinforcementLearner<RLParameters, 
						 Integer, 
						 SimpleAction, 
						 Feedback<Double>, 
						 SimpleStatelessPolicy> genericLearner;
	...
	protected void setUp() throws Exception {
		...
		reLearner = new RELearner<Integer, SimpleAction<Integer>>(reParams, policy);
		
		vreParams = new VREParameters(1.0, 0.65, 500, 0.45, 11235);
		vreLearner = new VRELearner<Integer, SimpleAction<Integer>>(vreParams, policy);
	}
	...
	/*
	 * Test method for 'edu.iastate.jrelm.rl.SimpleStatelessLearner.SimpleStatelessLearner(REParameters, Collection<O>)'
	 */
	public void testSimpleStatelessLearnerREParametersCollectionOfO() {
		simpleLearner = new SimpleStatelessLearner<Integer>(reParams, actionList);
	...
	/*
	 * Test method for 'edu.iastate.jrelm.rl.SimpleStatelessLearner.SimpleStatelessLearner(REParameters, SimpleActionDomain<O>)'
	 */
	public void testSimpleStatelessLearnerREParametersSimpleActionDomainOfO() {
		simpleLearner = new SimpleStatelessLearner<Integer>(reParams, simpleDomain);
		assertEquals(simpleLearner.getName(), reLearner.getName());
		comparePropensities_RELearner();
		compareProbabilities_RELearner();
		compareChooseAction_RELearner();
	...
	/*
	 * Test method for 'edu.iastate.jrelm.rl.SimpleStatelessLearner.SimpleStatelessLearner(REParameters, REPolicy<Integer, SimpleAction<O>>)'
	 */
	public void testSimpleStatelessLearnerREParametersREPolicyOfIntegerSimpleActionOfO() {		
		
		//Need to create two policies with the exact same information. Need separate policies, 
		//otherwise the SimpleStatless learner and RELearner will request new choices from the
		//same policy resulting in different choices for the same comparison iteration
		REPolicy<Integer, SimpleAction<Integer>> policy1 = 
			new REPolicy<Integer, SimpleAction<Integer>>(simpleDomain);
		REPolicy<Integer, SimpleAction<Integer>> policy2 = 
			new REPolicy<Integer, SimpleAction<Integer>>(simpleDomain);

		reLearner = new RELearner<Integer, SimpleAction<Integer>>(reParams, policy1);
	...
	/*
	 * Test method for 'edu.iastate.jrelm.rl.SimpleStatelessLearner.SimpleStatelessLearner(VREParameters, Collection<O>)'
	 */
	public void testSimpleStatelessLearnerVREParametersCollectionOfO() {
		simpleLearner = new SimpleStatelessLearner<Integer>(vreParams, actionList);
	...
	/*
	 * Test method for 'edu.iastate.jrelm.rl.SimpleStatelessLearner.SimpleStatelessLearner(VREParameters, SimpleActionDomain<O>)'
	 */
	public void testSimpleStatelessLearnerVREParametersSimpleActionDomainOfO() {
		simpleLearner = new SimpleStatelessLearner<Integer>(vreParams, simpleDomain);
		assertEquals(simpleLearner.getName(), vreLearner.getName());
		comparePropensities_VRELearner();
		compareProbabilities_VRELearner();
		compareChooseAction_VRELearner();
	...
	/*
	 * Test method for 'edu.iastate.jrelm.rl.SimpleStatelessLearner.SimpleStatelessLearner(VREParameters, REPolicy<Integer, SimpleAction<O>>)'
	 */
	public void testSimpleStatelessLearnerVREParametersREPolicyOfIntegerSimpleActionOfO() {
		...
		vreLearner = new VRELearner<Integer, SimpleAction<Integer>>(vreParams, policy1);
	...
	public void comparePropensities_RELearner(){
		for(int i=0; i < actionList.size(); i++){
			assertEquals(
              ((REPolicy<Integer, SimpleAction<Integer>>)simpleLearner.getPolicy()).getPropensities()[i], 
              reLearner.getPolicy().getPropensities()[i]);
		}
	}
	...
	public void compareProbabilities_RELearner(){
		for(int i=0; i < actionList.size(); i++){
			assertEquals(
			   ((REPolicy<Integer, SimpleAction<Integer>>)simpleLearner.getPolicy()).getProbabilities()[i], 
		       reLearner.getPolicy().getProbabilities()[i]);
		}
	}
...
//-------------------------	
	...
	public void comparePropensities_VRELearner(){
		for(int i=0; i < actionList.size(); i++){
			assertEquals(
              ((REPolicy<Integer, SimpleAction<Integer>>)simpleLearner.getPolicy()).getPropensities()[i], 
              vreLearner.getPolicy().getPropensities()[i]);
		}
	}
	...
	public void compareProbabilities_VRELearner(){
		for(int i=0; i < actionList.size(); i++){
			assertEquals(
			   ((REPolicy<Integer, SimpleAction<Integer>>)simpleLearner.getPolicy()).getProbabilities()[i], 
		       vreLearner.getPolicy().getProbabilities()[i]);
		}
	}

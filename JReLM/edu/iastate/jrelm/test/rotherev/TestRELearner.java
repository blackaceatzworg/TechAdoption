...
/**
 * The following tests are for the verification of the RELearner. That is,
 * these tests check that the RELearner is properly operating as learning 
 * component. 
 * 
 * These tests do not perform verification that RELearner correctly 
 * implements the Roth-Erev learning algorithm.
...
//}
	...
	/**
	 * This test checks to see that the propensities and probabilities
	 * of a selected action actually INCREASE with a POSITIVE reward and are 
	 * indeed GREATER THAN the propensities and probabilities of all unrewarded
	 * actions.
	 */
	public void testUpdatePositiveReward() {
		SimpleAction<String> selectedAction = learner.chooseAction();

		double oldProp = learner.getPolicy().getPropensity(selectedAction.getID());
		double oldProb = learner.getPolicy().getProbability(selectedAction.getID());
		
		learner.update(500.0);

		double newProp = learner.getPolicy().getPropensity(selectedAction.getID());
		double newProb = learner.getPolicy().getProbability(selectedAction.getID());
		...
		SimpleActionDomain<String> domain = 
			(SimpleActionDomain<String>) learner.getPolicy().getActionDomain();	
	...
	/**
	 * This test checks to see that the propensities and probabilities
	 * of a selected action actually DECREASE with a NEGATIVE reward and are 
	 * indeed LESS THAN the propensities and probabilities of all unrewarded
	 * actions.
	 */
	...
	/**
	 * This test checks to see that given a zero-valued reward, the 
	 * propensity of a rewarded action changes while the probability of
	 * the action stays the same. In addition, the propensities and 
	 * probabilities of all actions should be equal before and after the 
	 * zero-valued reward.
	 */
	...
	public void testGetInitialPropensity() {
		double initProp = ((REParameters)learner.getParameters()).getInitialPropensity();		
	...
	public void testSetPolicy() {
		REPolicy<Integer, SimpleAction<String>> newPolicy = 
			new REPolicy<Integer, SimpleAction<String>>(domain);
...
System.out.println("TestRELearner.testRandomSeeds");		
		...
		System.out.println("  	seed: " + seed + " first action: "+ learner.chooseAction().getID());

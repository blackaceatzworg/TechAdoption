...
//System.out.println("	newProb: "+newProb);
	...
	public void testUpdateActionSuccess() {
		...
		double oldProb = 
			learner.getPolicy().getProbability(actionToUpdate.getID());
		try {
...
//System.out.println("	newProb: "+newProb);
	...
	public void testUpdateActionFailure() {
		SimpleAction<String> actionToUpdate = 
			new SimpleAction<String>(domain.size() + 99, "Bad Action");
		FeedbackDoubleValue reward = new FeedbackDoubleValue(500.0);
...
//System.out.println("	"+ id +": "+action.getAct());
			switch(id){
				case 0: learner.update(-5000.0); break;
				case 1: learner.update(-5000.0); break;
				case 4: learner.update(150.0); break;
				case 7: learner.update(250.0); break;
				case 10: learner.update(-10.0); break;
				case 12: learner.update(5000.0); break;
				case 13: learner.update(5000.0); break;
				case 14: learner.update(100.0); break;
				case 15: learner.update(-100.0); break;
				case 16: learner.update(500.0); break;
				case 19: learner.update(-150.0); break;
				default: learner.update(100.0); break;
			}
		}
...
System.out.println("testRewardSequenceA()");
		...
		for(double j=10; j<=100; j=j+10){
		      ...
		      //Need to add a last action for almost 100% markup. Can't use %100 (1.0), since it 
		      //will result in an infinite price. The price calculation divides by 
		...
		VREParameters params = new VREParameters(100, 0.2757, 250.0, 0.505, 370115310);
		SimpleActionDomain<double[]> domain = new SimpleActionDomain<double[]>(actionList);

		assertEquals(domain.size(), 200);
		
		REPolicy<Integer, SimpleAction<double[]>> policy = 
			new REPolicy<Integer, SimpleAction<double[]>>(domain, 370115310);
		
		for(int i=0; i < domain.size(); i++)
			assertEquals(policy.getProbability(i), 1.0/ (double)domain.size());
		
		RELearner<Integer, SimpleAction<double[]>> learner = 
			new RELearner<Integer, SimpleAction<double[]>>(params, policy);
